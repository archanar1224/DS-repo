...
Program to find the Single source shortest path - Dijkstra
...
void dijkstra(int s, int n, int a[][n+1], int w[][n+1], int status[], int path[], int pri[]);
int main(int argc, char const *argv[])
{
	...
	scanf("%d", &source);
	dijkstra(source,n,a,w,status,path,pri);
...
void dijkstra(int s, int n, int a[][n+1], int w[][n+1], int status[], int path[], int pri[])
{
	...
	while(heap_size>0)
	{
		...
		for(v=1;v<=n;v++)
		{
			...
			else if(a[u][v]==1 && status[v]==1)
			{
				if(pri[v] > pri[u] + w[u][v])
				{
					pri[v] = pri[u] + w[u][v];
					path[v] = u;
				}
